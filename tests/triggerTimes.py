...
def getTriggerTimes(nevents, nsequence, ip, timeout=1000):
    '''
        ...
        while i < nevents:
            ...
            try:
                ...
                for channel in channels:
                    ...
                    triggerTimes[channel].append(trg_times)
...
if __name__ == '__main__':
    ...
    parser.add_option("-o", type="str", dest="output", help="output file", default=None)
    ...
    start = time.time()
    triggerTimes = getTriggerTimes(options.nevents, nsequence, options.ip)
    elapsed = time.time() - start

    if options.output:
        for ch, triggers in triggerTimes.items():
            with open(f'{options.output}_ch{ch}.txt', 'w') as f:
                for t in triggers:
                    for tt in t:
                        f.write(f'{tt}\n')
